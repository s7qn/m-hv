...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({
  ...
  return (
    <div id="student-dashboard-view" className="space-y-4 sm:space-y-6">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 rounded-2xl sm:rounded-3xl p-4 sm:p-8 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 sm:gap-6">
          <div className="space-y-1.5 sm:space-y-2">
            <div className="inline-flex items-center gap-2 px-2.5 sm:px-3 py-1 rounded-full bg-blue-500/20 border border-blue-400/30 text-blue-200 text-[11px] sm:text-xs font-bold">
          ...
          <div className="flex items-center gap-2 flex-wrap w-full md:w-auto">
            {totalQuizzesTaken === 0 && (
              <button
                ...
                className={`px-3 sm:px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all border shadow-sm cursor-pointer ${
                  showDemoSample
              ...
              >
                ...
                <span>{showDemoSample ? (language === 'ar' ? 'إخفاء العينة' : 'Hide Demo') : (language === 'ar' ? 'معاينة عينة' : 'Demo Stats')}</span>
              </button>
            )}

            <button
              onClick={onNavigateToLectures}
              className="px-3.5 sm:px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold flex items-center gap-1.5 transition-colors shadow-md shadow-blue-500/30 cursor-pointer"
            >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
        {/* KPI 1: Lectures Read */}
        <div id="kpi-lectures-read" className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-2xl p-3 sm:p-5 border border-slate-200/80 dark:border-slate-800 shadow-xs hover:border-blue-300 dark:hover:border-blue-600 transition-all space-y-2 sm:space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-blue-50 dark:bg-blue-950/70 text-blue-600 dark:text-blue-300 flex items-center justify-center font-bold shrink-0">
              <BookOpen className="w-4 h-4 sm:w-5 sm:h-5" />
            </div>
          </div>
          <div>
            <div className="flex items-baseline gap-1.5 sm:gap-2">
              <span className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white">{readLecturesCount}</span>
              <span className="text-[10px] sm:text-xs font-bold text-slate-400 dark:text-slate-500 truncate">/ {effectiveTotalLectures}</span>
            </div>
            {/* Progress Bar */}
            <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-1.5 sm:h-2 mt-2 sm:mt-3 overflow-hidden">
              <div
          ...
          <div className="flex items-center justify-between text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 pt-0.5 sm:pt-1">
            <span className="truncate">{language === 'ar' ? 'الإنجاز القرائي:' : 'Progress:'}</span>
            <span className="font-bold text-blue-700 dark:text-blue-400">%{readPercentage}</span>
          </div>
        </div>

        {/* KPI 2: Quiz Success Rate */}
        <div id="kpi-success-rate" className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-2xl p-3 sm:p-5 border border-slate-200/80 dark:border-slate-800 shadow-xs hover:border-emerald-300 dark:hover:border-emerald-600 transition-all space-y-2 sm:space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider truncate">
              {t.quizSuccessRateCard}
            </span>
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-emerald-50 dark:bg-emerald-950/70 text-emerald-600 dark:text-emerald-300 flex items-center justify-center font-bold shrink-0">
              <Award className="w-4 h-4 sm:w-5 sm:h-5" />
            </div>
          </div>
          <div>
            <div className="flex items-baseline gap-1.5 sm:gap-2">
              <span className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white">%{quizSuccessRate}</span>
              <span className="text-[10px] sm:text-xs font-bold text-emerald-600 dark:text-emerald-400 truncate">
                {quizSuccessRate >= 70 ? (language === 'ar' ? 'ممتاز' : 'Great') : (quizSuccessRate >= 50 ? (language === 'ar' ? 'جيد' : 'Good') : (language === 'ar' ? 'مراجعة' : 'Review'))}
              </span>
            </div>
            {/* Progress Bar */}
            <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-1.5 sm:h-2 mt-2 sm:mt-3 overflow-hidden">
          ...
          <div className="flex items-center justify-between text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 pt-0.5 sm:pt-1">
            <span>{language === 'ar' ? 'نجاح:' : 'Pass:'} <strong className="text-emerald-700 dark:text-emerald-400">{passedQuizzesCount}</strong></span>
            <span>{language === 'ar' ? 'رسوب:' : 'Fail:'} <strong className="text-rose-600 dark:text-rose-400">{failedQuizzesCount}</strong></span>
          </div>
        </div>

        {/* KPI 3: Total Quizzes Taken */}
        <div id="kpi-quizzes-taken" className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-2xl p-3 sm:p-5 border border-slate-200/80 dark:border-slate-800 shadow-xs hover:border-indigo-300 dark:hover:border-indigo-600 transition-all space-y-2 sm:space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider truncate">
              {t.totalQuizzesTakenCard}
            </span>
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-indigo-50 dark:bg-indigo-950/70 text-indigo-600 dark:text-indigo-300 flex items-center justify-center font-bold shrink-0">
              <GraduationCap className="w-4 h-4 sm:w-5 sm:h-5" />
            </div>
          </div>
          <div>
            <div className="flex items-baseline gap-1.5 sm:gap-2">
              <span className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white">{totalQuizzesTaken}</span>
              <span className="text-[10px] sm:text-xs font-bold text-slate-400 dark:text-slate-500 truncate">{language === 'ar' ? 'مؤمن' : 'Quizzes'}</span>
            </div>
            <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-1.5 sm:h-2 mt-2 sm:mt-3 overflow-hidden">
              <div
          ...
          <div className="flex items-center justify-between text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 pt-0.5 sm:pt-1">
            <span className="truncate">{language === 'ar' ? 'بيئة سرية' : 'Live'}</span>
            <button
              onClick={onNavigateToQuizzes}
              className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 font-bold cursor-pointer shrink-0"
            >
              {language === 'ar' ? 'دخول ←' : 'Browse →'}
            </button>
          </div>
        </div>

        {/* KPI 4: Average Score */}
        <div id="kpi-average-score" className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-2xl p-3 sm:p-5 border border-slate-200/80 dark:border-slate-800 shadow-xs hover:border-blue-300 dark:hover:border-blue-600 transition-all space-y-2 sm:space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider truncate">
              {t.averageScoreCard}
            </span>
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-blue-50 dark:bg-blue-950/70 text-blue-700 dark:text-blue-300 flex items-center justify-center font-bold shrink-0">
              <BarChart3 className="w-4 h-4 sm:w-5 sm:h-5" />
            </div>
          </div>
          <div>
            <div className="flex items-baseline gap-1.5 sm:gap-2">
              <span className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white">%{averageScore}</span>
              <span className="text-[10px] sm:text-xs font-bold text-blue-600 dark:text-blue-400 truncate">
          ...
          <div className="flex items-center justify-between text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 pt-0.5 sm:pt-1">
            <span className="truncate">{language === 'ar' ? 'المعدل التراكمي' : 'Cumulative'}</span>
