...
export const LecturesView: React.FC<LecturesViewProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Student Quick Stats Mini-Dashboard (Adaptive layout for phones and laptops) */}
      {lectures.length > 0 && (
        <div id="student-quick-dashboard-banner" className="bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 text-white rounded-2xl p-3.5 sm:p-5 shadow-sm border border-blue-800 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 sm:gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-white/10 border border-white/20 flex items-center justify-center shrink-0">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] sm:text-[10px] bg-blue-500/30 px-2 py-0.5 rounded-full border border-blue-400/30 font-bold text-blue-200">
                  Dashboard
                </span>
              </div>
          ...
          <div className="flex items-center gap-3 sm:gap-6 justify-between md:justify-end flex-wrap pt-2 md:pt-0 border-t md:border-t-0 border-white/10">
            {/* Reading Mini Progress */}
            <div className="text-right rtl:text-right ltr:text-left space-y-1">
              <div className="flex items-center justify-between gap-2 text-xs">
                <span className="text-slate-300 text-[11px] sm:text-xs">{language === 'ar' ? 'المقروءة:' : 'Read:'}</span>
              ...
              <div className="w-24 sm:w-36 bg-white/20 rounded-full h-1.5 sm:h-2 overflow-hidden">
                <div 
            ...
            {attemptsList.length > 0 && (
              <div className="text-right rtl:text-right ltr:text-left">
                <span className="text-[11px] sm:text-xs text-slate-300 block">{language === 'ar' ? 'النجاح:' : 'Success:'}</span>
            ...
            {onOpenDashboard && (
              <button
                ...
                className="px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold transition-colors flex items-center gap-1.5 whitespace-nowrap shrink-0 shadow-xs cursor-pointer ml-auto md:ml-0"
              >
                <BarChart3 className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-200" />
                <span>{language === 'ar' ? 'لوحة الإحصائيات' : 'Dashboard'}</span>
              </button>
            )}
          </div>
        </div>
      ...
      {/* Header & Filter Bar */}
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 sm:p-6 border border-blue-100 dark:border-slate-800 shadow-xs space-y-3 sm:space-y-4 transition-colors">
