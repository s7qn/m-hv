  ...
  return (
    ...
    >
      ...
      <div 
        ...
        className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl border border-blue-200 overflow-hidden flex flex-col my-auto max-h-[95vh]"
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 text-white px-5 py-3.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {phase === 'active' && (
              <button
                ...
                className="p-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-white text-xs font-medium flex items-center gap-1 transition-colors"
          ...
          <div className="p-6 sm:p-8 space-y-6 overflow-y-auto">
            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-3 pt-3 border-t border-blue-200/60 text-xs">
                    <div>
                      <span className="text-slate-500 block">{language === 'ar' ? 'عدد الأسئلة' : 'Questions'}</span>
                      <span className="font-bold text-blue-900">{quiz.questions.length} {language === 'ar' ? 'أسئلة' : 'Questions'}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block">{language === 'ar' ? 'مدة الاختبار' : 'Duration'}</span>
                      <span className="font-bold text-blue-900">{quiz.durationMinutes} {language === 'ar' ? 'دقائق' : 'Minutes'}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block">{language === 'ar' ? 'درجة النجاح' : 'Passing Grade'}</span>
                      <span className="font-bold text-emerald-700">%{quiz.passingScore}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-slate-100">
              <button
                ...
                onClick={onClose}
                className="w-full sm:w-auto px-5 py-2.5 rounded-xl border border-slate-300 text-slate-700 hover:bg-slate-100 font-bold text-xs sm:text-sm transition-colors"
              >
              ...
              <button
                ...
                className="w-full sm:w-auto px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-black text-xs sm:text-sm shadow-lg shadow-blue-500/25 flex items-center justify-center gap-2 transition-all"
              >
        ...
        {phase === 'active' && (
          <div className="p-4 sm:p-6 flex flex-col flex-1 overflow-y-auto space-y-4 select-none">
            ...
            <div className="bg-blue-50/70 border border-blue-200 rounded-xl p-3 flex flex-wrap items-center justify-between gap-3">
              {/* Question Progress */}
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-xs text-slate-500 font-semibold">{t.strikesCount}:</span>
                ...
                {(language === 'ar' ? currentQuestion.optionsAr : currentQuestion.optionsEn).map((opt, optIdx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-start p-3.5 rounded-xl border font-medium text-xs sm:text-sm transition-all flex items-center justify-between gap-3 ${
                        isSelected
            ...
            <div className="flex items-center justify-between gap-3 pt-2">
              <button
                ...
                className="px-4 py-2 rounded-xl border border-slate-200 hover:bg-slate-100 font-bold text-xs sm:text-sm text-slate-700 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1.5 transition-colors"
              >
              ...
              <div className="flex items-center gap-2">
                {isLastQuestion ? (
                  <button
                    id="btn-submit-exam"
                    onClick={submitExam}
                    className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs sm:text-sm shadow-md shadow-emerald-600/20 flex items-center gap-1.5 transition-all"
                  >
                  ...
                  <button
                    ...
                    onClick={() => setCurrentQuestionIndex(prev => prev + 1)}
                    className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs sm:text-sm shadow-md shadow-blue-500/20 flex items-center gap-1.5 transition-all"
                  >
              ...
              return (
                <div className="space-y-6">
                  ...
                  <div className={`p-6 rounded-2xl border text-center relative overflow-hidden ${
                    ...
                    <h3 className={`text-xl sm:text-2xl font-black ${isPassed ? 'text-emerald-800' : 'text-rose-800'}`}>
                      {isPassed ? t.passed : t.failed}
                    </h3>

                    <div className="flex items-center justify-center gap-6 mt-4">
                      <div>
                        <span className="text-xs text-slate-500 font-semibold block">{t.score}</span>
                        <span className="text-3xl font-black text-slate-900">%{pct}</span>
                      </div>
                      <div className="h-8 w-px bg-slate-200" />
                      <div>
                        <span className="text-xs text-slate-500 font-semibold block">{t.correctAnswers}</span>
                        <span className="text-xl font-black text-blue-900">{correct} / {total}</span>
                      </div>
                      <div className="h-8 w-px bg-slate-200" />
                      <div>
                        <span className="text-xs text-slate-500 font-semibold block">{t.strikesCount}</span>
                        <span className={`text-xl font-black ${strikes > 0 ? 'text-red-600' : 'text-slate-700'}`}>
                          {strikes}
                        </span>
                      </div>
                    </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="space-y-1 text-xs">
                            <div className="flex items-center gap-2">
                              <span className="text-slate-500 font-semibold">{t.yourAnswer}</span>
                              <span className={isCorrect ? 'text-emerald-700 font-bold' : 'text-rose-700 font-bold line-through'}>
                            ...
                            {!isCorrect && (
                              <div className="flex items-center gap-2">
                                <span className="text-slate-500 font-semibold">{t.correctAnswer}</span>
                          ...
                          <div className="bg-white/80 p-3 rounded-lg border border-slate-200/80 text-xs text-slate-700">
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-slate-100">
                    <button
                      id="btn-retake-quiz"
                      onClick={handleStartExam}
                      className="w-full sm:w-auto px-5 py-2.5 rounded-xl border border-slate-300 hover:bg-slate-100 text-slate-800 font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-colors"
                    >
                    ...
                    <button
                      id="btn-finish-quiz"
                      onClick={onClose}
                      className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-black text-xs sm:text-sm shadow-md shadow-blue-500/20 transition-all"
                    >
                      {t.backToLectures}
                    </button>
                  </div>
